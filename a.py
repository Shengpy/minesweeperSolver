...
def SolverCNF(cnf):
    ...
    # Khởi tạo assignments rỗng và gọi thuật toán DPLL
    assignments = {}
                    ...
                    if(len(cnf_clauses[j])==2 and cnf_clauses.count(cnf_clauses[j])==2):
                        ...
                        if(len(resolved_clause)==1):
                            if(resolved_clause[0]>0):
                               assignment[resolved_clause[0]]=True
                               cnf_clauses.append([resolved_clause[0]])
                            else:
                               assignment[abs(resolved_clause[0])]=False
                               cnf_clauses.append([resolved_clause[0]])
...
if __name__ == '__main__':
        ...
        if(choice == 1):
            ...
            output(board, solution,row,col,'Resolution'+str(row))
            
            
        #2.  brute force
        if(choice == 2):
            solution = brute_force_solver(cnf_clauses) 
            output(board, solution,row,col,'BruteForce'+str(row))
        #3  Backtracking solver
        if(choice == 3):
            solution = SolverCNF(cnf_clauses)
            solution = dict(sorted(solution.items(), key=lambda item: item[0]))
            output(board, solution,row,col,'Backtrack'+str(row))
        ...
        if(choice == 4):
            ...
            output(board, solution,row,col,'Pysat'+str(row))